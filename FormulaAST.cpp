...
namespace ASTImpl {
    ...
    public:
        ...
        virtual double Evaluate(const std::function<double(const Position*)>) const = 0;
    ...
    namespace {
        ...
        public:
            ...
            double Evaluate(const std::function<double(const Position*)> ExecuteDouble) const override {
                double result;
                switch (type_) {
                case Add:
                    result= lhs_->Evaluate(ExecuteDouble) + rhs_->Evaluate(ExecuteDouble);
                    break;
                case Subtract:
                    result= lhs_->Evaluate(ExecuteDouble) - rhs_->Evaluate(ExecuteDouble);
                    break;
                case Multiply:
                    ...
                    break;
                case Divide:
                    if (rhs_->Evaluate(ExecuteDouble) < 1e-6 && rhs_->Evaluate(ExecuteDouble) > -1e-6) {
        ...
        public:
            ...
            double Evaluate(const std::function<double(const Position*)> ExecuteDouble) const override {
                return type_ == UnaryMinus ? -operand_->Evaluate(ExecuteDouble) : operand_->Evaluate(ExecuteDouble);
            }
        ...
        public:
            ...
            double Evaluate(const std::function<double(const Position*)>) const override {
                return value_;
            }
...
double FormulaAST::Execute(const std::function<double(const Position*)> ExecuteDouble) const {
    return root_expr_->Evaluate(ExecuteDouble);
}
